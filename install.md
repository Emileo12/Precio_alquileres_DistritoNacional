# Precio Alquileres Distrito Nacional guide installation
...
activate precio_alquileres_distrito_nacional
```
...
To move beyond notebook prototyping, all reusable code should go into the `modulos/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `modulos` folder and use the modules inside your notebooks :
...
```python
from modulos.utils.paths import data_dir
data_dir()
```
